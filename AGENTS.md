...
`@sheplu/eslint-config-json` is a flat-config ESLint configuration for JSON, JSONC, and JSON5 files. It wraps [`@eslint/json`](https://github.com/eslint/json) and ships every non-deprecated upstream rule explicitly configured, so nothing is left to defaults. A scheduled CI job diffs the config against the upstream rule list and opens an issue on drift.

- Entry point: `index.js`
- Four exports:
    - `jsonRules` — array of flat-config objects (rule bundle only; supply your own `files` / `plugins` / `language`).
    - `defaultJsonConfig` / `defaultJsoncConfig` / `defaultJson5Config` — ready-to-drop-in flat-config objects for each language variant.
- Rule source: `src/eslint-json.js` (single category — `@eslint/json` is a flat rule list, not grouped).
...
- The package dogfoods its own config: `eslint.config.js` applies `defaultJsonConfig` / `defaultJsoncConfig` / `defaultJson5Config` to the repo's JSON files. `package.json`, `apkg.json`, and both lockfiles are `ignores`d — lockfiles are generated by tooling, and the two manifests follow ecosystem-conventional key order that fights `json/sort-keys`.

## When adding or changing rules

1. Add the rule to `src/eslint-json.js` with explicit options.
2. Keep every rule at `error` with spelled-out options unless there is a clear reason otherwise.
3. Run `npm test` — the upstream diff will catch renamed/removed rules, the offline plugin-drift test will catch rules missing from the installed `@eslint/json`.
